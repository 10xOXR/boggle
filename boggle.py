...
def neighbours_of_a_position(coords):
    # Gets the neighbours of a given position in the grid
    row = coords[0]
    col = coords[1]
...
def all_grid_neighbours(grid):
    # Gets all the possible neighbours for each position in the grid
    neighbours = {}
...
def path_to_word(grid, path):
    # Add all of the letters in a path to a string
    return ''.join([grid[p] for p in path])

def search(grid, dictionary):
    # Searches through the paths to locate words by matching strings to words in a dictionary
...
def display_words(words):
    ...
    with open('foundwords.txt', 'w+') as f:
        for word in words:
            f.write(word + "\n")
        f.write("Found %s words" % len(words))
